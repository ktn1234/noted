...
CREATE TABLE public.subscriptions (
	...
	CONSTRAINT subscriptions_pkey1 PRIMARY KEY (id, subscriber_user_id, user_id),
	CONSTRAINT subscriptions_subscriber_user_id_fkey FOREIGN KEY (subscriber_user_id) REFERENCES public.profiles(user_id),
	CONSTRAINT subscriptions_user_id_fkey1 FOREIGN KEY (user_id) REFERENCES public.profiles(user_id)
);
...
CREATE POLICY "Authenticated Users can ENABLE app notifications" ON public.notifications TO authenticated using (true);
CREATE POLICY "Authenticated Users can DISABLE own app notifications" on public.notifications TO authenticated using ( (( SELECT auth.uid() AS uid) = user_id) );
CREATE POLICY "Authenticated Users can READ own app notifications" ON public.notifications TO authenticated using ( (( SELECT auth.uid() AS uid) = user_id) );

-- public.subscriptions
CREATE POLICY "Authenticated Users can DELETE their user notifications" ON public.subscriptions TO authenticated using ( (( SELECT auth.uid() AS uid) = subscriber_user_id) );
CREATE POLICY "Authenticated Users can READ their user notifications" ON public.subscriptions TO authenticated using ( (( SELECT auth.uid() AS uid) = subscriber_user_id) );
CREATE POLICY "Authenticated Users can SUBSCRIBE for user notifications" ON public.subscriptions TO authenticated WITH CHECK (true);
